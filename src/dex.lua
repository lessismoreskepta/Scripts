...
local function main()
    ...
    -- Data table for enabling/disabling filters based on function existence
    local function hasFunction(funcName)
        return type(_G[funcName]) == "function"
    end

    Explorer.SearchFilters = { -- Now uses a data table to enable/disable filters if needed
        Enabled = {
            isa = true,
            remotes = true,
            bindables = true,
            rad = hasFunction("getnilinstances") and true or false, -- Example: only enable if getnilinstances exists
        },
		Comparison = {
...
Explorer.InitEntryTemplate = function()
    entryTemplate = create({
        ...
        -- Adding rounded corners to the Entry button
        {7,"UICorner",{
            Parent={1},
            CornerRadius=UDim.new(0, 4),
        }},
        -- Adding rounded corners to the Indent frame
        {8,"UICorner",{
            Parent={2},
            CornerRadius=UDim.new(0, 4),
        }},
        -- Adding hover effect to the Entry button
        {9,"UIStroke",{
            Parent={1},
            Color=Color3.new(0.04313725605607,0.35294118523598,0.68627452850342),
            Transparency=1,
            Thickness=1,
        }}
    })
...
local explorerItems = create({
    ...
    {12, "UICorner", {CornerRadius = UDim.new(0, 2), Parent = {2}}},
    {13, "UICorner", {CornerRadius = UDim.new(0, 2), Parent = {6}}},
    {14, "UICorner", {CornerRadius = UDim.new(0, 2), Parent = {8}}},
    {15, "UICorner", {CornerRadius = UDim.new(0, 2), Parent = {10}}},
    {16, "UICorner", {CornerRadius = UDim.new(0, 2), Parent = {11}}},
})
...
local function main()
	...
	Properties.DisplayBrickColorEditor = function(prop,entryIndex,col)
		...
		if not editor then
            ...
            editor.OnMoreColors:Connect(function()
                -- Special Case: BasePart.BrickColor to BasePart.Color
                editor:Close()
                local colProp
                local ok, err = pcall(function()
                    -- Find the "Color" property in BasePart
                    ...
                    if not colProp then
                        error("Could not find 'Color' property for BasePart")
                    end
                    -- If SavedColor is a BrickColor, get the Color3 value
                    local colorValue = editor.SavedColor
                    if typeof(colorValue) == "BrickColor" then
                        colorValue = colorValue.Color
                    elseif typeof(colorValue) == "table" and colorValue.Color then
                        colorValue = colorValue.Color
                    end
                    Properties.DisplayColorEditor(colProp, colorValue)
                end)
                if not ok then
                    warn("Failed to open Color editor from BrickColor: " .. tostring(err))
                end
            end)
    ...
    Properties.Init = function() -- Improved: Add UICorner to all UI elements
        local guiItems = create({
            ...
            {10,"Frame",{BackgroundColor3=Color3.new(0.15686275064945,0.15686275064945,0.15686275064945),BorderSizePixel=0,Name="ScrollCorner",Parent={1},Position=UDim2.new(1,-16,1,-16),Size=UDim2.new(0,16,0,16),Visible=false,}},
            {11,"Frame",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClipsDescendants=true,Name="List",Parent={1},Position=UDim2.new(0,0,0,23),Size=UDim2.new(1,0,1,-23),}},
            -- Add UICorner for ToolBar
            {12,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2},}},
            -- Add UICorner for SearchBox
            {13,"UICorner",{CornerRadius=UDim.new(0,4),Parent={4},}},
            -- Add UICorner for Reset button
            {14,"UICorner",{CornerRadius=UDim.new(0,4),Parent={6},}},
            -- Add UICorner for Refresh button
            {15,"UICorner",{CornerRadius=UDim.new(0,4),Parent={8},}},
            -- Add UICorner for ScrollCorner
            {16,"UICorner",{CornerRadius=UDim.new(0,4),Parent={10},}},
            -- Add UICorner for List
            {17,"UICorner",{CornerRadius=UDim.new(0,4),Parent={11},}},
            -- Add UICorner for SearchFrame
            {18,"UICorner",{CornerRadius=UDim.new(0,4),Parent={3},}},
        })
...
local function main()
	...
	ScriptViewer.Init = function()
		...
		copy.Size = UDim2.new(0.33,0,0,24)
		...
		save.Size = UDim2.new(0.34,0,0,24)
		...
		dumpbtn.Size = UDim2.new(0.33,0,0,24)
		...
		scriptNameLabel.Size = UDim2.new(1, 0, 0, 16)
		scriptNameLabel.Position = UDim2.new(0, 0, 1, -36)
		scriptNameLabel.TextColor3 = Settings.Theme.PlaceholderText or Color3.fromRGB(180, 180, 180)
		...
		scriptNameLabel.TextSize = 13
		scriptNameLabel.Text = ""
		scriptNameLabel.ClipsDescendants = true
		scriptNameLabel.ZIndex = 2
		scriptNameLabel.Parent = window.GuiElems.Content

		local decompLabel = Instance.new("TextLabel")
		decompLabel.BackgroundTransparency = 1
		decompLabel.Size = UDim2.new(1, 0, 0, 16)
		decompLabel.Position = UDim2.new(0, 0, 1, -20)
		decompLabel.TextColor3 = Color3.fromRGB(120, 200, 255)
		decompLabel.TextXAlignment = Enum.TextXAlignment.Left
		decompLabel.TextYAlignment = Enum.TextYAlignment.Center
		decompLabel.Font = Enum.Font.SourceSansItalic
		decompLabel.TextSize = 13
		decompLabel.Text = "  Improved By @lessismoreskepta"
		decompLabel.ClipsDescendants = true
		decompLabel.ZIndex = 2
		decompLabel.Parent = window.GuiElems.Content

		local oldViewScript = ScriptViewer.ViewScript
		ScriptViewer.ViewScript = function(scr)
			local success, source = pcall(env.decompile or function() end, scr)
			if not success or not source then
				...
				PreviousScr = scr
				scriptNameLabel.Text = " Script:  " .. scr:GetFullName()
			end
...
-- Lib Module: This module contains the main library functions and classes used throughout the DEX.
		...
		static.ShowWindow = function(window,data)
			...
			if align then
				...
				if align == nil and window.ClosedSide then -- Regular open
                    ...
                    window.GuiElems.Title.TextTransparency = 1
                    window.GuiElems.Minimize.ImageLabel.ImageTransparency = 1
                    window.GuiElems.Close.ImageLabel.ImageTransparency = 1
                    window.GuiElems.TopBar.BackgroundTransparency = 1
                    window.GuiElems.Outlines.ImageTransparency = 1

                    local ti = TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)
                    window:StopTweens()
                    window:DoTween(window.GuiElems.Main,ti,{Size = UDim2.new(0,window.SizeX,0,window.SizeY)})
                    window:DoTween(window.GuiElems.Title,ti,{TextTransparency = 0})
                    window:DoTween(window.GuiElems.Minimize.ImageLabel,ti,{ImageTransparency = 0})
                    window:DoTween(window.GuiElems.Close.ImageLabel,ti,{ImageTransparency = 0})
                    window:DoTween(window.GuiElems.TopBar,ti,{BackgroundTransparency = 0})
                    window:DoTween(window.GuiElems.Outlines,ti,{ImageTransparency = 0})
    ...
    Main.LoadSettings = function()
        ...
        if s and data and data ~= "" then
            ...
            if s and decoded then
                ...
                warn("Dex: Failed to decode settings, resetting to default.")
                Main.ResetSettings()
            end
        else
            Main.ResetSettings()
...
Main.CreateIntro = function(initStatus)
    local gui = create({
        ...
        {18,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="BetaLabel",Parent={2},Position=UDim2.new(1,-110,1,-45),Size=UDim2.new(0,105,0,20),Text="Beta",TextColor3=Color3.new(0.678431,0.847059,0.901961),TextSize=14,TextXAlignment=1,}},
    })
		Main.ShowGui(gui)
